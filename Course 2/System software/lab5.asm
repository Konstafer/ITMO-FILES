...
;       extrn   Word2s:far;Преобразует слово в его 10-чное представление с учетом 
			;знака в виде ASCIIZ-строки. Перед числом ставится
			;знак (-), либо пробел. Строка выравнивается вправо 
			;пробелами.
			;Вход:  ax - слово (знаковое)
			;       ds:si - указатель на буфер, куда будет помещена
			;               строка. Должно быть место для 7 
			;               символов (знак, 5 цифр, 0-терминатор)
...
aIn     DB       20 DUP(0ffh)
IN_LEN  equ     (($ - aIn) * 8 / 18)    ;Размер массива в 18-битовых словах
IN_B_LEN equ    ($ - aIn)
aOut    DB      ($ - aIn) DUP(?)
OUT_LEN equ     (($ - aOut) * 8 / 18)    ;Размер массива в 17-битовых словах
...
;       include lab5.mac
;               GET17BITW
;Получение в ax 17-битового слова из массива с расширением знака
...
;               PUT17BITW
;Сохранение 17-битового слова в eax в массиве таких слов
;Вход:  array - имя массива-битовой строки _без указания типа (type ptr)_
;       bx - номер слова в массиве 
;       eax - значение (учитывается только 17 бит)
;Выход: Нет
;Изменяет: Нет
...
Word2s  PROC    NEAR     ;Преобразует слово в его 10-чное представление с учетом 
			;знака в виде ASCIIZ-строки. Перед числом ставится
			;знак (-), либо пробел. Строка выравнивается вправо 
			;пробелами.
...
; ;       mov     bx,offset aOut
	...
	; sub     ecx,1
	; Bound_3:
		; call    Compare ;Менять ли элементы местами? (dl=0 - не менять)
		; test    dl,0FFh
		; jnz     Bound_3_skip
; ;               mov     ax,[bx]         ;Меняем
; ;               push    ax
...
; Bound_4:
	; cmp     wTmp,0  ;В wTmp - номер элемента с 0
	; je      Bound_End
	; inc     wTmp    ;В wTmp - номер элемента с 1
	; mov     eax,wTmp
	; mov     wBound,eax
	; jmp     Bound_2
